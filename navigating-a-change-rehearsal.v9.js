...
function setBandLabels(cfg){
  // 5 segments shown; map to our 5-band naming
  const bands = cfg.bands;
  el.b0.textContent = bands[0].name;
  el.b1.textContent = bands[1].name;
  el.b2.textContent = bands[2].name;
  el.b3.textContent = bands[3].name;
  el.b4.textContent = bands[4].name;
}
...
function setBar(elBar, elLine, value){
  ...
  elLine.style.left = `${clamp(value, 0, 100)}%`;
}
...
function bind(){
  ...
  // Sketch toggle
  const applySketch = (on) => document.documentElement.classList.toggle("sketch", !!on);
